...
chrome.runtime.onMessage.addListener(function(msg, sender, sendResponse){
	if(msg.text === "getwebzoom"){
		...
		chrome.storage.sync.get(["zoommagcircle", "zoommagsquare", "zoommagszoomlevel", "zoommagszoomsize", "zoommagszoomwidth", "zoommagszoomheight", "zoommagszoomsquare"], function(response){
			...
			zoommagszoomwidth = response.zoommagszoomwidth; if(zoommagszoomwidth == null)zoommagszoomwidth = zoommagszoomsize;
			zoommagszoomheight = response.zoommagszoomheight; if(zoommagszoomheight == null)zoommagszoomheight = zoommagszoomsize;
			zoommagszoomsquare = response.zoommagszoomsquare; if(zoommagszoomsquare == null)zoommagszoomsquare = false;
			if(zoommagcircle !== true && zoommagsquare !== true){ zoommagszoomwidth = 400; zoommagszoomheight = 400; }
			// For rectangular mode, honor different saved Width x Height values.
			// Older versions could leave zoommagszoomsquare=true in storage; only force
			// square for rectangles when the saved dimensions are already equal.
			if(zoommagcircle == true || (zoommagszoomsquare == true && zoommagsquare !== true)){ zoommagszoomheight = zoommagszoomwidth; }
...
function showmagnify(image){
	...
	circlewidth = parseInt(zoommagszoomwidth, 10) || parseInt(zoommagszoomsize, 10) || 200;
	circleheight = parseInt(zoommagszoomheight, 10) || circlewidth;
	if(zoommagcircle == true || (zoommagszoomsquare == true && zoommagsquare !== true)){ circleheight = circlewidth; }
	if(zoommagcircle !== true && zoommagsquare !== true){ circlewidth = 400; circleheight = 400; }
	...
	window.addEventListener("touchmove", moveSpot, {passive: false});
	// Set preventDefaultOnTouchMove to true when you want to prevent scrolling
	// and set it to false when you want to allow scrolling
...
function addmanify(image){
	...
	if(zoommagcircle == true){
		...
		// Rectangular mode should use the saved Width x Height without a circular mask.
		div.style.borderRadius = "0";
	}
	div.style.cursor = "none";
	div.style.width = circlewidth + "px";
	div.style.height = circleheight + "px";
	...
	div.style.top = parseInt(prevcoordy) - circleheight / 2 + "px";// previous
...
var zoommagcircle; var zoommagsquare; var zoommagszoomlevel; var zoommagszoomsize; var zoommagszoomwidth; var zoommagszoomheight; var zoommagszoomsquare;

document.addEventListener("DOMContentLoaded", function(){
	chrome.storage.sync.get(["zoommousescroll", "zoommousebuttonleft", "zoommousebuttonright", "zoommousescrollup", "zoommousescrolldown", "zoommagcircle", "zoommagsquare", "zoommagszoomlevel", "zoommagszoomsize", "zoommagszoomwidth", "zoommagszoomheight", "zoommagszoomsquare"], function(response){
