...
function clearActionPopup(tabId){
	try{
...
function enableMagnifyingGlassForTab(tab){
	if(!tab || tab.id == null)return;
	clearActionPopup(tab.id);
	chrome.tabs.sendMessage(tab.id, {text:"enablemagnifyingglass"}, function(){
		if(chrome.runtime.lastError){
			// Some pages, such as chrome:// pages, cannot receive content-script messages.
		}
	});
}

function openZoomPopupFromContext(){
	chrome.tabs.query({active: true, currentWindow: true}, function(tabs){
		var tab = tabs && tabs[0];
		try{
...
var openactiondoubleclick = function(tab){
	// Left-clicking the toolbar icon activates the magnifying glass.
	enableMagnifyingGlassForTab(tab);
};
...
// contextMenus
function getZoomValueFromContextMenuId(menuItemId){
	var str = String(menuItemId || "");
	if(str.indexOf("zoompage") == 0){
		return str.substring("zoompage".length);
	}
	if(str.indexOf("actionzoompage") == 0){
		return str.substring("actionzoompage".length);
	}
	return null;
}

async function onClickHandler(info){
	var str = info.menuItemId; var czl = getZoomValueFromContextMenuId(str); var reszoomin = str.substring(0, 8); var reszoomout = str.substring(0, 9); var reszoomreset = str.substring(0, 11);
	if(czl != null){
	...
	}else if(info.menuItemId == "totlopenpopupmenu"){
		openZoomPopupFromContext();
...
function getActionZoomLevelsForMenu(websitelevel){
	var levels = [];
	if(typeof websitelevel == "undefined" || websitelevel == null){
		websitelevel = ["85", "100", "115", "123"];
	}else if(typeof websitelevel === "string"){
		try{
	...
	if(Array.isArray(websitelevel)){
		for(var i = 0; i < websitelevel.length; i++){
			var v = parseInt(websitelevel[i], 10);
			if(!isNaN(v) && v > 0 && v <= 400 && levels.indexOf(v) === -1){ levels.push(v); }
		}
	}else{
		for(var key in websitelevel){
			var val = parseInt(websitelevel[key], 10);
			if(isNaN(val)){ val = parseInt(key, 10); }
			if(!isNaN(val) && val > 0 && val <= 400 && levels.indexOf(val) === -1){ levels.push(val); }
		}
	}

	if(levels.indexOf(100) === -1){ levels.push(100); }
	levels.sort(function(a, b){ return a - b; });
	return levels;
}

function createActionZoomLevelMenu(parentId){
	chrome.storage.sync.get(["websitelevel"], function(items){
		var levels = getActionZoomLevelsForMenu(items.websitelevel);
		var chunkSize = 12;

		if(levels.length <= chunkSize){
			for(var i = 0; i < levels.length; i++){
				chrome.contextMenus.create({
					"title": levels[i] + "%",
					"type": "normal",
					"id": "actionzoompage" + levels[i],
					"contexts": ["action"],
					"parentId": parentId
				});
			}
		...
		for(var start = 0; start < levels.length; start += chunkSize){
			var end = Math.min(start + chunkSize, levels.length);
			var groupId = "actionzoomgroup" + start;
			chrome.contextMenus.create({
				"title": levels[start] + "–" + levels[end - 1] + "%",
				"type": "normal",
				"id": groupId,
				"contexts": ["action"],
				"parentId": parentId
...
if(chrome.contextMenus){
	if(actionmenuadded == false){
		...
		var contexts = ["action"];
		var zoomLevelParent = browsercontext("Zoom level", "totlactionzoomlevels", "");
		createActionZoomLevelMenu(zoomLevelParent);
		browsercontext("Open full zoom menu", "totlopenpopupmenu", "");
...
// Firefox Android
// Keep the action popup disabled so chrome.action.onClicked is fired for the
// left-click magnifying-glass action. The popup is temporarily enabled only
// when the user chooses "Open zoom menu" from the right-click action menu.
