...
function fitPredefinedButtonsToPopup(){
	var predefinedbuttons = document.getElementById("predefinedbuttons");
	if(!predefinedbuttons || predefinedbuttons.className == "hidden")return;

	// Keep the original popup width while the preset buttons fit. If the preset
	// row becomes wider, enlarge the popup up to Chrome's practical popup limit;
	// any remaining overflow stays accessible with horizontal scrolling.
	requestAnimationFrame(function(){
		var popupClass = document.documentElement.className || "modern";
		var defaultWidths = {small: 297, large: 319, modern: 300};
		var defaultWidth = defaultWidths[popupClass] || defaultWidths.modern;
		var maxPopupWidth = 780;
		var leftEdge = Math.ceil(predefinedbuttons.getBoundingClientRect().left);
		var rightPadding = 10;
		var desiredWidth = leftEdge + predefinedbuttons.scrollWidth + rightPadding;
		var newWidth = Math.max(defaultWidth, Math.min(maxPopupWidth, desiredWidth));

		document.documentElement.style.width = newWidth + "px";
		document.body.style.width = newWidth + "px";

		// Make the content column follow the widened popup but do not force it
		// beyond the popup when Chrome caps the available width.
		var zbox = document.querySelector("." + popupClass + " .zbox");
		if(zbox && popupClass == "modern"){
			zbox.style.width = Math.max(280, newWidth - 20) + "px";
		}
	});
}
...
document.addEventListener("DOMContentLoaded", function(){
	...
	$("circle").addEventListener("click", function(){
		chrome.storage.sync.set({"zoommagcircle": true, "zoommagsquare": false, "zoommagszoomsquare": true}, function(){
	...
	$("square").addEventListener("click", function(){
		chrome.storage.sync.set({"zoommagcircle": false, "zoommagsquare": true, "zoommagszoomsquare": false}, function(){
			chrome.tabs.query({active: true, currentWindow: true},
				function(tabs){
					if(tabs[0]){
						chrome.tabs.sendMessage(tabs[0].id, {text: "enablemagnifyingglass"});
	...
	chrome.storage.sync.get(["darkmode", "allzoom", "allzoomvalue", "websitezoom", "badge", "steps", "lightcolor", "zoomchrome", "zoomweb", "largepopup", "zoombydomain", "zoombypage", "zoombyregex", "defaultallscreen", "defaultsinglescreen", "screenzoom", "zoomfont", "smallpopup", "largepopup", "modernpopup", "prezoombutton", "websitelevel"], function(response){
		...
		if(prezoombutton == true){
			...
			enableHorizontalWheelScroll(predefinedbuttons);
		}
